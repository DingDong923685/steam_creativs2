...
Format: {size_prompt}""", model="gpt-4o", temperature=1.0)
                    ...
                    with st.spinner(f"Generating image {completed_images_count} for '{topic}'..."):
                        if template == 5:
                            ...
                            image_prompt = chatGPT(f"Generate a visual image description 50 words MAX for {topic}, candid moment unstaged, taken in the moment by eye witness like with a smartphone, viral reddit style, make it dramatic and visually enticing", model='o1-mini')
                        elif template == 8:
                            ...
                            image_prompt = chatGPT(f"""Generate a visual image description 15 words MAX for {topic}.
                        ...
                        if template == 5:
                            ...
                            image_url = gen_flux_img_lora(image_prompt)
                        elif template == 8:
                            st.text("mayyyyy" + image_prompt)
                            image_url = gen_flux_img(image_prompt, width=720, height=480)
                        elif template == 9:
                            st.text("mayyyyy" + image_prompt)
                            image_url = gen_flux_img(image_prompt, width=720, height=480)
                        else:
